...
import { FiArrowLeft, FiMail, FiPhone, FiMapPin, FiSlash, FiCheckCircle, FiEdit } from 'react-icons/fi';
...
const CustomerDetail = () => {
  ...
  const handleToggleBan = async () => {
    const action = customer.isBanned ? 'BỎ ẨN' : 'ẨN';
    if (window.confirm(`Bạn có chắc muốn ${action} khách hàng này?`)) {
      try {
        await customersAPI.ban(id);
        fetchCustomerDetail(); // Tải lại dữ liệu mới nhất
    ...
    <MainLayout title="Chi Tiết Khách Hàng">
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow p-6 relative overflow-hidden">
            ...
            <div className={`w-24 h-24 rounded-full flex items-center justify-center text-3xl font-bold ${(customer.isBanned || customer.is_banned) ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
              {(customer.name || customer.fullName || customer.email || 'K')?.charAt(0).toUpperCase()}
            </div>
            
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-2xl font-bold text-gray-800">{customer.name || customer.fullName || customer.email || 'Chưa đặt tên'}</h2>
                <button
                  onClick={() => navigate(`/admin/customers/${id}/edit`)}
                  className="flex items-center space-x-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition"
                >
                  <FiEdit className="w-4 h-4" />
                  <span>Chỉnh sửa hồ sơ khách hàng</span>
                </button>
              </div>
              <div className="space-y-3 text-gray-600">
                <div className="flex items-center">
                  ...
                  <span>{customer.region || customer.address?.city || 'Chưa cập nhật'}</span>
                </div>
                <div className="flex items-center">
                ...
                <div className="flex items-center">
                  ...
                  <span>{customer.phoneNumber || customer.phone || 'Chưa có SĐT'}</span>
                </div>
                <div className="flex items-center">
        ...
        {/* Khu vực mở rộng: Lịch sử đơn hàng (Nếu bạn muốn làm thêm) */}
        <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-bold mb-4">Lịch sử hoạt động</h3>
            <p className="text-gray-500 italic">Tính năng hiển thị lịch sử đơn hàng đang được cập nhật...</p>
        </div>
      </div>
    </MainLayout>
  );
