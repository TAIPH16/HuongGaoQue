...
const ProductForm = () => {
  ...
  const fetchProduct = async () => {
    try {
      ...
      console.log('Fetched product details:', product.details);
      setFormData({
        name: product.name || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Luôn gửi details, kể cả khi là array rỗng
      ...
      console.log('Submitting details:', detailsToSend);
      console.log('Details count:', detailsToSend.length);
      ...
      if (isEdit) {
        ...
        setSuccessModal({ isOpen: true, message: 'Cập nhật sản phẩm thành công, thông tin sẽ được cập nhật sau ít phút' });
      } else {
        await productsAPI.create(formDataToSend);
        setSuccessModal({ isOpen: true, message: 'Thêm sản phẩm thành công, thông tin sẽ được cập nhật sau ít phút' });
      }
  ...
  return (
    <MainLayout title={isEdit ? 'Chỉnh sửa sản phẩm' : 'Thêm sản phẩm'}>
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="space-y-4">
                ...
                {/* Display additional custom details */}
                {formData.details?.filter(d => !['Loại hạt', 'Độ ẩm', 'Chỉ số dẻo', 'Protein', 'Tỉ lệ hạt gãy', 'Chỉ số đường', 'Chất xơ'].includes(d.indexName)).map((detail, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    <div className="flex-1">
                      <label className="block text-sm font-medium text-gray-700 mb-2">{detail.indexName}</label>
                      {detail.type === 'select' ? (
                        <select
                          value={detail.value || ''}
                          onChange={(e) => {
                            const updatedDetails = [...formData.details];
                            updatedDetails[formData.details.findIndex(d => d.indexName === detail.indexName)].value = e.target.value;
                            setFormData({ ...formData, details: updatedDetails });
                          }}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none"
                        >
                  ...
                  />
                    ...
                    <button
                      type="button"
                      onClick={() => removeDetail(formData.details.findIndex(d => d.indexName === detail.indexName))}
                ...
                <div className="border-t pt-4">
                  ...
                  <input
                    type="text"
                      value={newDetail.indexName}
                      onChange={(e) => setNewDetail({ ...newDetail, indexName: e.target.value })}
