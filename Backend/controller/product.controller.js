...
const addVariationToProduct = async (req, res) => {
  try {
    const { productId } = req.params;
    const variation = req.body;

    // QUAN TRỌNG: Tìm theo trường 'productId' chứ KHÔNG PHẢI '_id'
    const product = await Product.findOne({ productId: productId });

    if (!product) return res.status(404).json({ message: "Product not found" });

    // Đề phòng db cũ không có mảng variations
    ...
    await product.save();
    res.status(201).json({ variations: product.variations });
  } catch (err) {
    console.error(err);
